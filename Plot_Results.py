...
def Plot_Kfold():
    ...
    for n in range(eval.shape[0]):
        for j in range(len(Graph_Term)):
            ...
            X = np.arange(5)
            ax.bar(X + 0.00, Graph[:, 5], edgecolor='k', hatch='//', color='r', width=0.10, label="KNN")
            ax.bar(X + 0.10, Graph[:, 6], edgecolor='k', hatch='-', color='#6dedfd', width=0.10, label="XGBoost")
            ax.bar(X + 0.20, Graph[:, 7], edgecolor='k', hatch='//', color='lime', width=0.10, label="SVM")
            ax.bar(X + 0.30, Graph[:, 8], edgecolor='k', hatch='-', color='#ed0dd9', width=0.10, label="SGRU")
            ax.bar(X + 0.40, Graph[:, 9], edgecolor='w', hatch='..', color='k', width=0.10, label="FIGO-ASGRU-AM")
...
def Plot_Epoch():
    ...
    for n in range(eval.shape[0]):
        for j in range(len(Graph_Term)):
            ...
            ax = fig.add_axes([0.1, 0.1, 0.8, 0.8])
            X = np.arange(5)
            ax.bar(X + 0.00, Graph[:, 5], edgecolor='k', hatch='//', color='b', width=0.10, label="KNN")
            ax.bar(X + 0.10, Graph[:, 6], edgecolor='k', hatch='-', color='#6dedfd', width=0.10, label="XGBoost")
            ax.bar(X + 0.20, Graph[:, 7], edgecolor='k', hatch='//', color='lime', width=0.10, label="SVM")
            ax.bar(X + 0.30, Graph[:, 8], edgecolor='k', hatch='-', color='#ed0dd9', width=0.10, label="SGRU")
            ax.bar(X + 0.40, Graph[:, 9], edgecolor='w', hatch='..', color='k', width=0.10, label="FIGO-ASGRU-AM")
...
def Plot_Feature():
    ...
    for n in range(eval.shape[0]):
        for j in range(len(Graph_Term)):
            ...
            ax = fig.add_axes([0.1, 0.1, 0.8, 0.8])
            X = np.arange(4)
            ax.bar(X + 0.00, Graph[:, 5], edgecolor='k', hatch='-', color='#ff028d', width=0.10, label="KNN")
            ax.bar(X + 0.10, Graph[:, 6], edgecolor='k', hatch='-', color='#be03fd', width=0.10, label="XGBoost")
            ax.bar(X + 0.20, Graph[:, 7], edgecolor='k', hatch='-', color='#0485d1', width=0.10, label="SVM")
            ax.bar(X + 0.30, Graph[:, 8], edgecolor='k', hatch='-', color='#fd3c06', width=0.10, label="SGRU")
            ax.bar(X + 0.40, Graph[:, 9], edgecolor='w', hatch='', color='k', width=0.10, label="FIGO-ASGRU-AM")
            ...
            plt.legend(loc='upper center', bbox_to_anchor=(0.5, 1.15),
                       ncol=3, fancybox=True, shadow=True)
            path = "./Results/Dataset_%s_Feature_%s_Med.png" % (n + 1, Terms[Graph_Term[j]])
            plt.savefig(path)
            plt.show()
